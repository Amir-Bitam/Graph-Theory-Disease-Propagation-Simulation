...
def interactions_minimales(G, source, cible):
    try:
        chemin_min = nx.shortest_path_length(G, source=source, target=cible)
        print(f"✅ Le virus mettra au **minimum {chemin_min} interaction(s)** pour atteindre le sommet {cible} depuis {source}.")
        return chemin_min
